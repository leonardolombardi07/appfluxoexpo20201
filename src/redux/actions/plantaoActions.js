...
export const checkPlantaoStatus = () => async (dispatch) => {
  ...
  let canChangePlantaoStatus = false;

  if (currentTime.getMinutes() > 50 || currentTime.getMinutes() < 20) {
    if (!token) {
      ...
      try {
        ...
        if (horarios.length === 0) {
          //Nao foi encontrado, o plantão foi aberto há muito tempo
          canChangePlantaoStatus = true;
    ...
    //currentTime.getMinutes() < 20 || currentTime.getMinutes() > 50
    //currentTime.getMinutes() > 20 && currentTime.getMinutes() < 50
  } else if (20 <= currentTime.getMinutes() <= 50) {
    canChangePlantaoStatus = false;
    if (!token) {
      statusPlantao = "Fechado";
    } else if (token) {
...
export const fecharPlantao = (qrCodeURL) => async (dispatch) => {
  ...
  try {
    if (qrCodeURL !== "https://fluxoconsultoria.poli.ufrj.br/appfluxoeobicho")
      throw "Por gentileza, scannear o QR code localizado na sede da Fluxo";
...
export const abrirPlantao = (qrCodeURL) => async (dispatch) => {
  try {
    ...
    if (
      postID ===
      "Não é possível abrir um plantão antes de 6AM ou depois de 17AM"
    )
      throw "Não é possível abrir um plantão antes de 6AM ou depois de 17AM";
    const json = JSON.stringify({
      primeiro_plantao: postID,
      segundo_plantao: null,
    }); // lidar com marcação de dois plantoes
